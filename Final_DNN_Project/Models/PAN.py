...
def train(model, train_loader, train_loader_masks, optimizer, loss_criteria, num_epochs=10, verbose=False):
    ...
    for epoch in range(num_epochs):
        ...
        if verbose:
            ...
            print("Train accuracy(iou):", acc / num_of_batchs)
...
if __name__ == "__main__":
    ...
    backbones = ['resnet34', 'timm-regenty_320', 'resnet101']
    encoder_output_stride = [8, 16, 32]
    ...
    for backbone in backbones:
        for output_stride, decoder_channel in zip(encoder_output_stride, decoder_channels):
                ...
                for batch_size_train in batch_sizes:
                    for optimizer in range(num_of_optimizers):
                        for loss_criteria in loss_criterias:
                            ...
                            model = smp.PAN(encoder_name=backbone, decoder_channels=decoder_channel,
                            ...
                            if acc_val > best_acc:
                                ...
                                best_output_stride = output_stride
                                best_decoder_channel = decoder_channel
                                best_batch_size = batch_size_train
                                best_loss_criteria = loss_criteria
                                best_optimizer = optimizer

                            results.append([acc_val, backbone, output_stride, decoder_channel, epoch, batch_size_train, loss_criteria, optimizer])

                            print("Best accuracy until now:", best_acc)
                            print("Best backbone until now:", best_backbone)
                            print("Best output stride until now:", best_output_stride)
    ...
    model = smp.PAN(encoder_name=best_backbone, decoder_channels=best_decoder_channel, in_channels=3, classes=1, activation="sigmoid")
    ...
    for i in range(len(test_output)):
        ...
        plt.subplot(1, 4, 1), plt.title('Original Image'), plt.axis('off'), plt.imshow(test_output[i][0].detach().numpy()[0, 0, :, :].T, cmap='gray')
        plt.subplot(1, 4, 2), plt.title('Mask'), plt.axis('off'), plt.imshow(test_output[i][1].detach().numpy()[0, 0, :, :].T, cmap='gray')
        plt.subplot(1, 4, 3), plt.title('Predict'), plt.axis('off'), plt.imshow(test_output[i][2].detach().numpy()[0, 0, :, :].T, cmap='gray')
        plt.show()
