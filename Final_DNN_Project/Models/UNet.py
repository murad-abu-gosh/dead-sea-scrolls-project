...
if __name__ == "__main__":
    ...
    backbones = ['resnet34', 'resnet101' 'timm-regnety_320']
    encoder_depths = [4, 5, 6]
    decoder_channels = [[128, 64, 32, 16], [256, 128, 64, 32, 16], [512, 256, 128, 64, 32, 16]]
    num_of_epcohs = [10]
    batch_sizes = [5, 10, 20]
    ...
    best_loss_criteria = smp.losses.TverskyLoss(mode='binary')
    best_optimizer = -1
                ...
                for batch_size_train in batch_sizes:
                    for optimizer in range(num_of_optimizers):
                        for loss_criteria in loss_criterias:
                            ...
                            model = smp.Unet(encoder_name = backbone, encoder_depth = encoder_depth, decoder_channels = decoder_channel,
    ...
    model = smp.Unet(encoder_name = best_backbone, encoder_depth = best_encoder_depth, decoder_channels = best_decoder_channel,
                            in_channels = 3, classes = 1, activation = "sigmoid", decoder_use_batchnorm = True)
