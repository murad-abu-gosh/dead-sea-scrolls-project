...
# data_path = "/home/mihmadkh/PycharmProjects/Final_DNN_Project/DataSets/DataSet_1280x720"
...
if __name__ == "__main__":
                ...
                for batch_size_train in batch_sizes:
                    for optimizer in range(num_of_optimizers):
                        for loss_criteria in loss_criterias:
                            ...
                            epochs_loss, acc, epoch_list = train(model, train_loader=train_loader,
                                                                 train_loader_masks=train_loader_masks,
                                                                 optimizer=optimizers[optimizer],
                                                                 loss_criteria=loss_criteria, num_epochs=epoch,
                                                                 verbose=True)
    ...
    train_losses, acc_train, epoch_list = train(model, train_loader=train_validation_loader,
                                                train_loader_masks=train_validation_loader_masks,
                                                optimizer=best_optimizers[best_optimizer],
                                                loss_criteria=best_loss_criteria, num_epochs=best_num_of_epochs,
                                                verbose=True)
    ...
    for i in range(len(test_output)):
        plt.subplot(1, 4, 1), plt.axis('off'), plt.imshow(test_output[i][0].cpu().detach().numpy()[0, 0, :, :].T, cmap='gray')
        plt.subplot(1, 4, 2), plt.axis('off'), plt.imshow(test_output[i][1].cpu().detach().numpy()[0, 0, :, :].T, cmap='gray')
        plt.subplot(1, 4, 3), plt.axis('off'), plt.imshow(test_output[i][2].cpu().detach().numpy()[0, 0, :, :].T, cmap='gray')
        plt.show()
