...
def extract_pieces(img_name):
    # Source image
    source_image_path = "mask_" + img_name
    source_image_name, ext = source_image_path.split(".")
    vanilla_img_path = img_name
    vanilla_img_name, ext2 = vanilla_img_path.split(".")
    ...
    for index, contour in enumerate(selected_contours):
        ...
        masked_image = cv2.bitwise_and(vanilla_img, source_image, mask=mask)
